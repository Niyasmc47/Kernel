...
export default function RootAccessSection() {
  return (
    <section 
      id="root-access" 
      className="relative w-full min-h-[90vh] flex items-center justify-center py-28 md:py-36 bg-[#040203] text-gray-200 overflow-hidden"
    >
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 sm:px-8 flex flex-col items-center text-center">
        ...
        <AnimatedContent direction="down" className="mb-6">
          <div className="inline-flex items-center space-x-3 px-4 py-1.5 rounded-full bg-red-950/50 border border-red-500/30 backdrop-blur-md">
            <span className="w-2 h-2 rounded-full bg-red-500 animate-ping" />
            <span className="font-mono text-xs tracking-wider text-red-400 uppercase font-semibold">
              THE OPPOSING FORCE • ROOT ACCESS
            </span>
          </div>
        </AnimatedContent>

        <AnimatedContent direction="up" delay={0.2} className="mb-8 max-w-3xl">
          <h2 className="font-cinematic text-5xl sm:text-6xl lg:text-7xl font-normal tracking-tight text-white mb-6 drop-shadow-[0_4px_24px_rgba(239,68,68,0.4)]">
            <GlitchText text="ROOT ACCESS." className="text-red-500" />
          </h2>
          <p className="font-cinematic text-2xl sm:text-3xl text-red-200/90 italic font-light">
            "Some things were never meant to be rebuilt."
          </p>
        </AnimatedContent>

        <AnimatedContent direction="up" delay={0.4} className="max-w-2xl text-center">
          <div className="font-sans text-base sm:text-lg text-gray-300/90 leading-relaxed space-y-4 mb-10">
            <p>
          ...
          <div className="p-6 rounded-2xl bg-black/60 border border-red-500/20 backdrop-blur-md inline-block max-w-lg text-left">
            <div className="font-mono text-xs text-red-400/90 font-semibold tracking-wider uppercase mb-2">
              TWO OPPOSING BELIEFS
            </div>
            <div className="font-sans text-sm text-gray-300 leading-relaxed">
              <span className="text-emerald-400 font-medium">KERNEL:</span> "Fix what is broken. Honor the fracture."<br />
              <span className="text-red-400 font-medium">ROOT ACCESS:</span> "Delete the flawed. Impose the clean state."
            </div>
          </div>
        </AnimatedContent>
